...
// import { sendOTP } from '../../../backend/controller/authController'
...
function ForgetPassword() {
    ...
    // for step-1: enter email and send OTP
        const sendOTP = async()=>{
            setLoading(true)
            try{
                ...
                toast.error(error.response.data.message || "Error in sending OTP. Please try again.")
                setLoading(false)
            }
        }
    // for step-2: enter OTP and verify
    const verifyOTP = async()=>{
        ...
        try{
            ...
            toast.error(error.response.data.message || "Error in verifying OTP. Please try again.")
            setLoading(false)
        }
    }
    ...
    <div className='min-h-screen flex items-center justify-center bg-gray-100 px-4'>
        ...
        {step == 1 && <div className='bg-white shadow-md rounded-xl p-8 max-w-md w-full'>
            ...
            <form className='space-y-4' onSubmit={(e)=>e.preventDefault()}>
                ...
                <input id='email'type='email' className='mt-1 w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-[black]' placeholder='you@example.com' required onChange={(e)=>setEmail(e.target.value)} value={email}/>
                </div>
        ...
        {step == 2 && <div className='bg-white shadow-md rounded-xl p-8 max-w-md w-full'>
            ...
            <form className='space-y-4' onSubmit={(e)=>e.preventDefault()}>
                ...
                <input id='otp'type='text' placeholder='* * * *' required onChange={(e)=>setOtp(e.target.value)} value={otp} className='mt-1 w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-[black]'/>
        ...
        {step == 3 && <div className='bg-white shadow-md rounded-xl p-8 max-w-md w-full'>
            ...
            <form className='space-y-4'onSubmit={(e)=>e.preventDefault()}>
                ...
                <input id='password'type='password' placeholder='********' required onChange={(e)=>setNewPassword(e.target.value)} value={newPassword} className='mt-1 w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-[black]'/>
               </div>
               <div>
                <label htmlFor='conPassword' className='block text-sm font-medium text-gray-700'>Confirm Password</label>
                <input id='conPassword'type='password' placeholder='********' required onChange={(e)=>setConPassword(e.target.value)} value={conPassword} className='mt-1 w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-[black]'/>
               </div> 

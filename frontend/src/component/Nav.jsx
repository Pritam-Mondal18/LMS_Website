        ...
        <div>
            <div className="w-[100%] h-[70px] fixed top-0 px-[20px] py-[10px] flex items-center justify-between bg-[#00000047] z-10">
                ...
                <div className="w-[30%] lg:flex items-center justify-center gap-4 hidden">
                    ...
                    {/* {!userData && <IoPersonCircle className="w-[50px] h-[50px] text-white cursor-pointer" onClick={()=>setShow(prev=>!prev)}/>}
                    {userData && userData?.photoUrl ? <img src={userData?.photoUrl} alt="Profile" className="w-[50px] h-[50px] rounded-full text-white flex items-center justify-center text-[20px] border-2 bg-black border-white cursor-pointer" onClick={()=>setShow(prev=>!prev)}/>: <div className="w-[50px] h-[50px] rounded-full text-white flex items-center justify-center text-[20px] border-2 bg-black border-white cursor-pointer" onClick={()=>setShow(prev=>!prev)}>
                        {userData?.name?.slice(0,1)?.toUpperCase()}
                    </div>} */}
                    {userData ? (userData.photoUrl ? (<img src={userData.photoUrl}alt="Profile" className="w-[50px] h-[50px] rounded-full border-2 bg-black border-white cursor-pointer"/>) : (<div className="w-[50px] h-[50px] rounded-full text-white flex items-center justify-center border-2 bg-black border-white cursor-pointer">{userData?.name?.[0]?.toUpperCase() || "?"}</div>)) : (<IoPersonCircle className="w-[50px] h-[50px] text-white cursor-pointer" />)}
